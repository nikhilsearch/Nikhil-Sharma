...
serve(async (req) => {
  ...
  try {
    ...
    const { id, title, content_markdown, excerpt, cover_image_url, author_name, slug, status } = await req.json();

    // Validate required fields
    if (!id || !title || !content_markdown || !slug) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: id, title, content_markdown, slug' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
